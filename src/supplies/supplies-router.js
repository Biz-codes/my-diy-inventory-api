...
suppliesRouter
  ...
  .post(jsonParser, (req, res, next) => {
    ...
    SuppliesService.insertSupply(req.app.get("db"), newSupply)
      .then((supply) => {
        res
          .status(201)
...
suppliesRouter.route("/my-supplies/:user_id").all((req, res, next) => {
...
suppliesRouter
  .route("/:supply_id")
  .all((req, res, next) => {
    SuppliesService.getById(req.app.get("db"), req.params.supply_id)
      .then((supply) => {
        if (!supply) {
          return res.status(404).json({
  ...
  .delete((req, res, next) => {
    SuppliesService.deleteSupply(req.app.get("db"), req.params.supply_id)
