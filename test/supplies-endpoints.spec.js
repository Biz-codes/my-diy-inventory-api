...
describe("Supplies Endpoints", function () {
  ...
  before("clean the table", () =>
    db.raw("TRUNCATE users, supplies RESTART IDENTITY CASCADE")
  );
    ...
    context("Given there are supplies in the database", () => {
      ...
      beforeEach("insert supplies", () => {
        return db
          ...
          .insert(testUsers)
          .then(() => {
            return db.into("supplies").insert(testSupplies);
          });
      });

      it("responds with 200 and all of the supplies", () => {
        return supertest(app).get("/api/supplies").expect(200, testSupplies);
      });
    });

    context(`Given an XSS attack supply`, () => {
      const testUsers = makeUsersArray();
      const { maliciousSupply, expectedSupply } = makeMaliciousSupply();
      ...
      beforeEach("insert malicious supply", () => {
        return db
          ...
          .insert(testUsers)
          .then(() => {
            return db.into("supplies").insert([maliciousSupply]);
          });
      });
      ...
      it("removes XSS attack content", () => {
        return supertest(app)
          ...
          .expect(200)
          .expect((res) => {
            expect(res.body[0].user_id).to.eql(expectedSupply.user_id);
  ...
  describe(`POST /api/supplies`, () => {
    ...
    it(`creates a supply, responding with 201 and the new supply`, function () {
      ...
      return supertest(app)
        ...
        .then((res) =>
          supertest(app).get(`/api/supplies/${res.body.id}`).expect(res.body)
        );
    });
    ...
    it("removes XSS attack content", () => {
      ...
      return supertest(app)
        .post(`/api/supplies`)
        .send(maliciousSupply)
        .expect(201)
        .expect((res) => {
          expect(res.body.user_id).to.eql(expectedSupply.user_id);
          expect(res.body.supply_name).to.eql(expectedSupply.supply_name);
          expect(res.body.details).to.eql(expectedSupply.details);
          expect(res.body.quantity).to.eql(expectedSupply.quantity);
        });
    });
  });
  ...
  describe(`DELETE /api/supplies/:supply_id`, () => {
    ...
    context("Given there are supplies in the database", () => {
      ...
      it("responds with 204 and removes the supply", () => {
        const idToRemove = 2;
        const expectedSupply = testSupplies.filter(
          (supply) => supply.id !== idToRemove
        );
        return supertest(app)
          .delete(`/api/supplies/${idToRemove}`)
          .expect(204)
          .then((res) =>
            supertest(app).get(`/api/supplies`).expect(expectedSupply)
          );
      });
    });
  });
